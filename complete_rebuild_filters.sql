...
-- 重置自增ID
ALTER TABLE filter_types AUTO_INCREMENT = 1;
ALTER TABLE filter_options AUTO_INCREMENT = 1;

-- 重新插入 filter_types
INSERT INTO filter_types (id, name, code, index_position, is_active, sort_order, created_at, updated_at) VALUES
(1, '排序', 'sort', 1, 1, 1, NOW(), NOW()),
(2, '类型', 'type', 2, 1, 2, NOW(), NOW()),
(3, '地区', 'region', 3, 1, 3, NOW(), NOW()),
(4, '语言', 'language', 4, 1, 4, NOW(), NOW()),
(5, '年份', 'year', 5, 1, 5, NOW(), NOW()),
(6, '状态', 'status', 6, 1, 6, NOW(), NOW());
...
LEFT JOIN filter_options fo ON ft.id = fo.filter_type_id AND fo.is_active = 1
