...
| `status` | string | ✅ | 仅用于软删除与辅助维护：当传 `deleted` 时触发软删除；当传 `completed/on-going` 时仅用于维护 `isCompleted`，不再持久化字符串状态到系列。系列状态请使用下方 `statusOptionName` 指定。 | "on-going" / "completed" / "deleted" |
...
###### 关于 status 字段（是否还需要传入？）
- 结论：目前“需要”。`status` 不是系列表的持久化字段，但在 Ingest 接口仍有两项用途：
  - 软删除：传 `deleted` 将把系列标记为软删除（`isActive=0, deletedAt=当前时间`）。
  - 维护完结态：传 `completed`/`on-going` 用来维护系列的 `isCompleted` 布尔值，便于生成 `upStatus` 文案（如“已完结”或“更新至第X集”）。
- 与 `statusOptionName` 的关系：系列状态（如“连载中/已完结”等）对外展示与筛选一律来源于 `statusOptionName`/`status_option_id`，而非该 `status` 字符串。
- 示例：
  - 新增/更新正常系列（未删除）：`status: "on-going"` 或 `status: "completed"`（根据是否完结选择）
  - 软删除系列：`status: "deleted"`
